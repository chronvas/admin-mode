...
namespace admin_mode.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, EnrollmentDate = DateTime.Now};
                var createUserResult = await UserManager.CreateAsync(user, model.Password);
                
                //if (result.Succeeded && addtoroleresult.Succeeded)
                if (createUserResult.Succeeded )
                {
                    // with registration, add the user just registered to the default role "Users"
