...
namespace admin_mode.Controllers {
    ...
    public class AdminMainPageController : Controller {
        #region Index (has Request.ContentType)
        // GET: AdminMainPage
        [Authorize(Roles = "Global Admin")]
        public ActionResult Index() {
            if (Request.ContentType == "application/json") { //this will return a json (or whatever we want) if in the Header of the request the Content-type is application/jsonn
        ...
        public ActionResult Users(string sortOrder , string currentFilter , string searchString , int? page) {
            ...
            var users = from s in db.Users
                        select s;
            if (!String.IsNullOrEmpty(searchString)) { //search in emails and usernames
            ...
            var pagedlistUsers = users.ToPagedList(pageNumber, pageSize);
            db.Dispose();
        ...
        //switcher according to the request's content-type
        [HttpGet]
        public ActionResult API() {
            if (Request.ContentType.Contains("json")) {
                return RedirectToAction("UsersToJson");
            }
            if (Request.ContentType.Contains("xml")) {
                return RedirectToAction("UsersToXml");
            }
            return HttpNotFound("error in field Content-Type of the request");
        }
